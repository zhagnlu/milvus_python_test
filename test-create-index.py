...
str_prefix =""
print(str_prefix)
...
def init_collection():
  ...
  while(index < 4):
    entities = [
        ...
        [int(random.randrange(0, 1000000)) for _ in range(3000)],  # field int1
  ...
  hello_milvus.create_index("string1", index_name="string1_index", _async=True)
...
def load_collection():
  ...
  hello_milvus.create_index("string1", "string1_index")
  hello_milvus.load()
  print("load done")
...
def search():
  ...
  #print(len(result))
  expr_str = " int1 == 1000"
  result = hello_milvus.query(expr=expr_str) 
...
def query():
  ...
  schema = CollectionSchema(fields, "hello_milvus is the simplest demo to introduce the APIs")
  hello_milvus = Collection("hello_milvus", schema)
  perform_queries(hello_milvus, 100)
