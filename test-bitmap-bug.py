...
fields = [
    FieldSchema(name="pk", dtype=DataType.INT64, is_primary=True, auto_id=False),
        #FieldSchema(name="pk", dtype=DataType.VARCHAR, is_primary=True, max_length=1000, auto_id=False),
...
entities = [
        ...
        #[str(i) for i in range(3000)],  # field pk
    ...
    # [[str(random.randint(0, 10000) % 20) for _ in range(5) ] for _ in range(3000)], #array 2
...
#index_param = { "index_type" : "BITMAP", 'indexoffsetcache.nabled': False}
...
hello_milvus.create_index("pk", index_name='xx', index_params=index_param)
...
index = 0
while(index < 30):
  entities[0] = [(i + index * 3000) for i in range(3000)]
...
time.sleep(5)
#hello_milvus.alter_index('xx', {'indexoffsetcache.enabled': False})
client = MilvusClient(uri='http://localhost:19530')
print(client.describe_index("hello_small", "xx"))
hello_milvus.load()
print("load done")
...
while index < 10:
  ...
  #result = hello_milvus.query(expr="string1 like \"xxx5000\"", output_fields=["string1"])
  result = hello_milvus.query(expr="pk % 10 == 3", limit=10)
  #print(result)
  print(len(result))
  index +=1
  #time.sleep(1)
