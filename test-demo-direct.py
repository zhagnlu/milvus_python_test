...
fields = [
    ...
    FieldSchema(name="partition", dtype=DataType.VARCHAR, max_length=200, nullable=True),
...
def init_collection():
  ...
  hello_milvus = Collection("hello_milvus", schema, shards_num=1, enable_dynamic_field=False)
  ...
  while(index < 10):
    entities = [
        ...
        # [i for i in range(100 *index, 100 * (index + 1))],  # field int2
        [ random.choice(string_pool) for i in range(10000)],  # field random
...
if __name__ == "__main__":
  ...
  while (1):
     ...
     expr = "partition == 'xxx'"
    #  filter_params = {"xx": ["xxx", "xxxx"]}
     #print (expr)
     #result = hello_milvus.query(expr=expr, output_fields=["count(*)"], expr_params={"use_set_for_term_in": "true"})
     #print(f"Query with string pool (1000 strings): {len(result)} results")
     vectors_to_search = [[random.random() for _ in range(128)] for _ in range(2)]
     search_params_hnsw = {
        "metric_type": "L2",
        "params": {"ef": 10},
     }
     result = hello_milvus.search(vectors_to_search, "embeddings", search_params_hnsw, limit=100, expr = expr) 
     print(result)
  # while(1):
